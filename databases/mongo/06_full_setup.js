...
["v_employees_with_department", "v_department_summary", "v_department_age_stats", "v_empty_departments",
 "employees", "departments", "users"].forEach((c) => {
  try { db[c].drop(); } catch (e) { /* ignore */ }
});
...
db.employees.createIndex({ departmentId: 1 }, { name: "idx_employees_department_id" });
db.employees.createIndex({ email: 1 }, { name: "idx_employees_email" });
db.employees.createIndex({ lastName: 1, firstName: 1 }, { name: "idx_employees_last_first" });
db.employees.createIndex({ age: 1 }, { name: "idx_employees_age" });
db.employees.createIndex({ departmentId: 1, lastName: 1, firstName: 1 }, { name: "idx_employees_dept_name" });
...
const departments = [
  ...
  "Operations", "Customer Support", "Product Management", "Legal", "Research & Development"
...
const deptMap = {};
departments.forEach((d, i) => { deptMap[d.name] = deptIds[i]; });
...
const employees = empData.map(([fn, ln, email, age, dept]) => ({
  firstName: fn, lastName: ln, email, age: NumberInt(age),
  departmentId: deptMap[dept], departmentName: dept,
...
db.createView("v_employees_with_department", "employees", [
  ...
  { $project: { _id:1, firstName:1, lastName:1, email:1, age:1, department: { _id:"$department._id", name:"$department.name" } } }
]);
...
db.createView("v_department_age_stats", "employees", [
  { $group: { _id: "$departmentId", departmentName: { $first: "$departmentName" }, employeeCount: { $sum:1 }, minAge: { $min:"$age" }, maxAge: { $max:"$age" }, avgAge: { $avg:"$age" } } },
  { $project: { _id:0, departmentId:"$_id", departmentName:1, employeeCount:1, minAge:1, maxAge:1, avgAge: { $round:["$avgAge",1] } } },
...
print(`  indexes:     ${db.departments.getIndexes().length + db.employees.getIndexes().length + db.users.getIndexes().length} total`);
